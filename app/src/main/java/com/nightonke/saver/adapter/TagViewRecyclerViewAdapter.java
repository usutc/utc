...
import com.nightonke.saver.model.UtcCoin;
...
public class TagViewRecyclerViewAdapter
    ...
    private List<List<UtcCoinRecord>> contents;
    ...
    public TagViewRecyclerViewAdapter(List<UtcCoinRecord> coCoinRecords, Context context, int position) {
        ...
        if (!IS_EMPTY) {
            ...
            Collections.sort(coCoinRecords, new Comparator<UtcCoinRecord>() {
                @Override
            ...
            double monthSum = 0;
            double yearSum = 0;
            List<UtcCoinRecord> yearSet = new ArrayList<>();
            List<UtcCoinRecord> monthSet = new ArrayList<>();

            for (UtcCoinRecord coCoinRecord : coCoinRecords) {
            ...
            if (chartType == PIE) {
                ...
                for (int i = 0; i < contents.size(); i++) {
                    ...
                    for (UtcCoinRecord coCoinRecord : contents.get(i)) {
                ...
                if (IS_EMPTY) {
                    ...
                    holder.from.setText(
                            mContext.getResources().getString(R.string.from) + " " +
                                    startYear + " " + UtcCoinUtil.GetMonthShort(startMonth));
                    holder.sum.setText(UtcCoinUtil.GetInMoney((int)Sum));
                    holder.to.setText(
                            mContext.getResources().getString(R.string.to) + " " +
                                    endYear + " " + UtcCoinUtil.GetMonthShort(endMonth));
                    holder.to.setTypeface(UtcCoinUtil.GetTypeface());
                    holder.from.setTypeface(UtcCoinUtil.GetTypeface());
                }
                holder.sum.setTypeface(UtcCoinUtil.typefaceLatoLight);
                break;
            case TYPE_CELL:
                ...
                holder.expanse.setTypeface(UtcCoinUtil.GetTypeface());
                switch (chartType) {
                    case PIE:
                        ...
                        if (type.get(position - 1).equals(SHOW_IN_MONTH)) {
                            holder.date.setText(year + " " + UtcCoinUtil.GetMonthShort(month));
                        } else {
                            holder.date.setText(year + " ");
                        }

                        holder.expanse.setText(UtcCoinUtil.GetInMoney((int) (double) SumList.get(position - 1)));
                        ...
                        if (type.get(position - 1).equals(SHOW_IN_YEAR)) {
                            ...
                            for (int i = 0; i < numColumns; i++) {
                                ...
                                SubcolumnValue value = new SubcolumnValue(
                                        ...
                                        UtcCoinUtil.GetRandomColor());
                                value.setLabel(UtcCoinUtil.MONTHS_SHORT[month] + " " + year);
                            ...
                            for (int i = 0; i < numColumns; i++) {
                                axisValueList.add(new AxisValue(i)
                                        .setLabel(UtcCoinUtil.GetMonthShort(i + 1)));
                            }
                            ...
                            for (int i = 0; i < numColumns; ++i) {
                                ...
                                SubcolumnValue value = new SubcolumnValue((float)
                                        DayExpanseSum[(year - startYear) * 372
                                                + (month - 1) * 31 + i],
                                        UtcCoinUtil.GetRandomColor());
                                value.setLabel(UtcCoinUtil.MONTHS_SHORT[month] + " " + (i + 1) + " " + year);
                                subcolumnValues.add(value);
                                Column column = new Column(subcolumnValues);
                                column.setHasLabels(false);
                                column.setHasLabelsOnlyForSelected(false);
        ...
        public void onValueSelected(final int columnIndex, int subColumnIndex, SubcolumnValue value) {
            Snackbar snackbar =
                    Snackbar.with(mContext)
                            ...
                            .margin(15, 15)
                            .backgroundDrawable(UtcCoinUtil.GetSnackBarBackground(fragmentTagId))
                            .textColor(Color.WHITE)
                            .textTypeface(UtcCoinUtil.GetTypeface())
                            .actionLabel(mContext.getResources().getString(R.string.check))
                            .actionLabelTypeface(UtcCoinUtil.typefaceLatoLight)
            ...
            if (fragmentPosition == SUM_HISTOGRAM) {
                if (type.get(position).equals(SHOW_IN_MONTH)) {
                    ...
                    timeString = " " + UtcCoinUtil.GetMonthShort(month)
                            + " " + (columnIndex + 1) + " "
                    ...
                    if ("zh".equals(UtcCoinUtil.GetLanguage())) {
                        text = mContext.getResources().getString(R.string.on) + timeString + "\n" +
                                UtcCoinUtil.GetSpendString((int) value.getValue());
                        dialogTitle = mContext.getResources().getString(R.string.on) + timeString +
                                UtcCoinUtil.GetSpendString((int) value.getValue());
                    } else {
                        text = UtcCoinUtil.GetSpendString((int) value.getValue()) + "\n" +
                                mContext.getResources().getString(R.string.on) + timeString;
                        dialogTitle = UtcCoinUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString;
                    }
                    snackbar.text(text);
                    snackbar.actionListener(new ActionClickListener() {
                        ...
                        public void onActionClicked(Snackbar snackbar) {
                            List<UtcCoinRecord> shownUtcCoinRecords = new ArrayList<>();
                            boolean isSamed = false;
                            for (UtcCoinRecord coCoinRecord : contents.get(position)) {
                                if (coCoinRecord.getCalendar().get(Calendar.DAY_OF_MONTH) == columnIndex + 1) {
                                    shownUtcCoinRecords.add(coCoinRecord);
                                    isSamed = true;
                                } else {
                                    if (isSamed) {
                                        break;
                ...
                if (type.get(position).equals(SHOW_IN_YEAR)) {
                    ...
                    timeString = " " + UtcCoinUtil.GetMonthShort(columnIndex + 1) + " "
                            + timeString.substring(timeString.length() - 4, timeString.length());
                    if ("zh".equals(UtcCoinUtil.GetLanguage())) {
                ...
                if (type.get(position).equals(SHOW_IN_MONTH)) {
                    ...
                    if ("zh".equals(UtcCoinUtil.GetLanguage())) {
                        text = mContext.getResources().getString(R.string.on) + timeString +
                                UtcCoinUtil.GetSpendString((int) value.getValue()) + "\n" +
                                "于" + UtcCoinUtil.GetTagName(contents.get(position).get(0).getTag());
                        dialogTitle = mContext.getResources().getString(R.string.on) + timeString +
                                UtcCoinUtil.GetSpendString((int) value.getValue()) + "\n" +
                                "于" + UtcCoinUtil.GetTagName(contents.get(position).get(0).getTag());
                    } else {
                        text = UtcCoinUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString + "\n"
                                + "in " + UtcCoinUtil.GetTagName(contents.get(position).get(0).getTag());
                        dialogTitle = UtcCoinUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString + "\n"
                                + "in " + UtcCoinUtil.GetTagName(contents.get(position).get(0).getTag());
                    }
            ...
            Snackbar snackbar =
                    Snackbar
                            ...
                            .actionListener(new ActionClickListener() {
                                ...
                                public void onActionClicked(Snackbar snackbar) {
                                    List<UtcCoinRecord> shownUtcCoinRecords = new ArrayList<>();
                                    for (UtcCoinRecord record : contents.get(position)) {
                                        if (record.getTag() == tagId) {
                                            shownUtcCoinRecords.add(record);
                                        }
                                    }
                                    ((FragmentActivity) mContext).getSupportFragmentManager()
                                            .beginTransaction()
