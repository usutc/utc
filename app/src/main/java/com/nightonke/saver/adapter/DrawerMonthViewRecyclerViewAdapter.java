...
public class DrawerMonthViewRecyclerViewAdapter
    ...
    public void onBindViewHolder(final viewHolder holder, final int position) {
        holder.month.setText(UtcCoinUtil.GetMonthShort(months.get(position) + 1));
        holder.month.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);

        holder.year.setText(years.get(position) + "");
        holder.year.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);

        holder.sum.setText(UtcCoinUtil.getInstance().GetInRecords(records.get(position)));
        holder.sum.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);

        holder.money.setText(UtcCoinUtil.getInstance().GetInMoney((int) (double) (expenses.get(position))));
        holder.money.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
    }
