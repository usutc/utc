...
public class TodayViewRecyclerViewAdapter
    ...
    private ArrayList<UtcCoinRecord> allData;
    ...
    public TodayViewRecyclerViewAdapter(int start, int end, Context context, int position) {
        ...
        if (!IS_EMPTY) {
            ...
            for (int i = 0; i < size; i++) {
                UtcCoinRecord coCoinRecord = allData.get(i);
                ...
                if (axis_date == Calendar.DAY_OF_WEEK) {
                    if (UtcCoinUtil.WEEK_START_WITH_SUNDAY)
    ...
    public void onBindViewHolder(final viewHolder holder, final int position) {
        ...
        switch (getItemViewType(position)) {
            case TYPE_HEADER:
                ...
                holder.expanseSum.setText(UtcCoinUtil.GetInMoney((int) Sum));

                holder.date.setTypeface(UtcCoinUtil.GetTypeface());
                holder.dateBottom.setTypeface(UtcCoinUtil.GetTypeface());
                holder.expanseSum.setTypeface(UtcCoinUtil.typefaceLatoLight);

                if (IS_EMPTY) {
                    holder.emptyTip.setVisibility(View.VISIBLE);
                    holder.emptyTip.setText(UtcCoinUtil.GetTodayViewEmptyTip(fragmentPosition));
                    ...
                    if (!(fragmentPosition == TODAY || fragmentPosition == YESTERDAY)) {
                        ...
                        for (int i = 0; i < columnNumber; i++) {
                            ...
                            SubcolumnValue value = new SubcolumnValue(
                                    originalTargets[i], UtcCoinUtil.GetRandomColor());
                        ...
                        for (int i = 0; i < columnNumber; i++) {
                            axisValueList.add(
                                    new AxisValue(i).setLabel(UtcCoinUtil.GetAxisDateName(axis_date, i)));
                        }
                        ...
                        public void onValueSelected(int p, SliceValue sliceValue) {
                            ...
                            Snackbar snackbar =
                                    Snackbar
                                            ...
                                            .margin(15, 15)
                                            .backgroundDrawable(UtcCoinUtil.GetSnackBarBackground(
                                                    fragmentPosition - 2))
                                            .text(text)
...
// histogram data///////////////////////////////////////////////////////////////////////////////////
                                ...
                                for (int i = Expanse.get(tagId).size() - 1; i >= 0; i--) {
                                    UtcCoinRecord coCoinRecord = Expanse.get(tagId).get(i);
                                    if (axis_date == Calendar.DAY_OF_WEEK) {
                                        if (UtcCoinUtil.WEEK_START_WITH_SUNDAY) {
...
// set value touch listener of histogram////////////////////////////////////////////////////////////
                        holder.histogram.setOnValueTouchListener(
                            ...
                            public void onValueSelected(int columnIndex,
                                ...
                                String text = UtcCoinUtil.GetSpendString((int) value.getValue());
                                if (tagId != -1)
                                    // belongs a tag
                                    if ("zh".equals(UtcCoinUtil.GetLanguage()))
...
// set the listener of the show all button//////////////////////////////////////////////////////////
            ...
            case TYPE_BODY:
                ...
                holder.tagImage.setImageResource(
                        UtcCoinUtil.GetTagIcon(allData.get(position - 1).getTag()));
                ...
                holder.cell_date.setTypeface(UtcCoinUtil.typefaceLatoLight);
                ...
                holder.layout.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        dialog = new MaterialDialog.Builder(mContext)
                                .icon(UtcCoinUtil.GetTagIconDrawable(allData.get(position - 1).getTag()))
...
// set the listener of the check button on the snack bar of histogram///////////////////////////////
    private class mActionClickListenerForHistogram implements ActionClickListener {
        @Override
        public void onActionClicked(Snackbar snackbar) {
            ArrayList<UtcCoinRecord> shownUtcCoinRecords = new ArrayList<>();
            int index = timeIndex;
            if (axis_date == Calendar.DAY_OF_WEEK) {
                if (UtcCoinUtil.WEEK_START_WITH_SUNDAY) index++;
                else
                    if (index == 6) index = 1;
                    else index += 2;
            }
            ...
            if (tagId != -1) {
                for (int i = 0; i < Expanse.get(tagId).size(); i++)
                    if (Expanse.get(tagId).get(i).getCalendar().get(axis_date) == index)
                        shownUtcCoinRecords.add(Expanse.get(tagId).get(i));
            } else {
                for (int i = 0; i < allData.size(); i++)
                    if (allData.get(i).getCalendar().get(axis_date) == index)
                        shownUtcCoinRecords.add(allData.get(i));
            }
            ((FragmentActivity)mContext).getSupportFragmentManager()
                    .beginTransaction()
                    .add(new RecordCheckDialogFragment(
        ...
        switch (fragmentPosition) {
            ...
            case THIS_WEEK:
                ...
                return mContext.getResources().getString(R.string.on)
                        + UtcCoinUtil.GetWeekDay(timeIndex);
            case LAST_WEEK:
                // 在上周一
                // on last Monday
                return mContext.getResources().getString(R.string.on)
            ...
            case THIS_MONTH:
                ...
                // on Jan. 1
                return mContext.getResources().getString(R.string.on) +
                        UtcCoinUtil.GetMonthShort(month) + UtcCoinUtil.GetWhetherBlank() +
                        (timeIndex + 1) + UtcCoinUtil.GetWhetherFuck();
            case LAST_MONTH:
                // 在1月1日
                // on Jan. 1
                return mContext.getResources().getString(R.string.on) +
            ...
            case THIS_YEAR:
                if ("zh".equals(UtcCoinUtil.GetLanguage()))
                    ...
                    return mContext.getResources().getString(R.string.in) +
                            ...
                            UtcCoinUtil.GetMonthShort(timeIndex + 1);
                else
                    // in Jan. 1
                    return mContext.getResources().getString(R.string.in) +
                            UtcCoinUtil.GetMonthShort(timeIndex + 1) + " " +
...
// set the dateString of this fragment//////////////////////////////////////////////////////////////
    private void setDateString() {
        ...
        Calendar yesterday = UtcCoinUtil.GetYesterdayLeftRange(today);
        basicTodayDateString = "--:-- ";
        basicTodayDateString += UtcCoinUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
                + " " + today.get(Calendar.DAY_OF_MONTH) + " " +
                today.get(Calendar.YEAR);
        basicYesterdayDateString = "--:-- ";
        basicYesterdayDateString += UtcCoinUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
        ...
        switch (fragmentPosition) {
            ...
            case YESTERDAY:
                ...
                break;
            case THIS_WEEK:
                Calendar leftWeekRange = UtcCoinUtil.GetThisWeekLeftRange(today);
                Calendar rightWeekRange = UtcCoinUtil.GetThisWeekRightShownRange(today);
                dateString = UtcCoinUtil.GetMonthShort(leftWeekRange.get(Calendar.MONTH) + 1)
                        + " " + leftWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftWeekRange.get(Calendar.YEAR) + " - " +
                        UtcCoinUtil.GetMonthShort(rightWeekRange.get(Calendar.MONTH) + 1)
                ...
                break;
            case LAST_WEEK:
                Calendar leftLastWeekRange = UtcCoinUtil.GetLastWeekLeftRange(today);
                Calendar rightLastWeekRange = UtcCoinUtil.GetLastWeekRightShownRange(today);
                dateString
                        = UtcCoinUtil.GetMonthShort(leftLastWeekRange.get(Calendar.MONTH) + 1)
                        + " " + leftLastWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftLastWeekRange.get(Calendar.YEAR) + " - " +
                        UtcCoinUtil.GetMonthShort(rightLastWeekRange.get(Calendar.MONTH) + 1)
                ...
                break;
            case THIS_MONTH:
                dateString = UtcCoinUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
                        + " " + today.get(Calendar.YEAR);
                dateShownString
                ...
                break;
            case LAST_MONTH:
                Calendar lastMonthCalendar = UtcCoinUtil.GetLastMonthLeftRange(today);
                dateString
                        = UtcCoinUtil.GetMonthShort(lastMonthCalendar.get(Calendar.MONTH) + 1)
            ...
            case THIS_YEAR:
                ...
                break;
            case LAST_YEAR:
                Calendar lastYearCalendar = UtcCoinUtil.GetLastYearLeftRange(today);
    ...
    private String getAllDataDialogTitle() {
        ...
        if ("zh".equals(UtcCoinUtil.GetLanguage())) {
            ...
            postfix = UtcCoinUtil.GetSpendString((int)Sum);
        } else {
            prefix = UtcCoinUtil.GetSpendString((int)Sum);
            postfix = "";
        }
        switch (fragmentPosition) {
            case TODAY:
