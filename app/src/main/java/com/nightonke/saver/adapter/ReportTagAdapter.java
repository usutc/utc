...
public class ReportTagAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        icon.setImageDrawable(UtcCoinUtil.getInstance().GetTagIconDrawable((int)tagExpense.get(position + 1)[2]));
        name.setText(UtcCoinUtil.getInstance().GetTagName((int)tagExpense.get(position + 1)[2]) + UtcCoinUtil.getInstance().GetPurePercentString(tagExpense.get(position + 1)[1] * 100));
        expense.setText(UtcCoinUtil.getInstance().GetInMoney((int)tagExpense.get(position + 1)[0]));
        records.setText(UtcCoinUtil.getInstance().GetInRecords((int)tagExpense.get(position + 1)[3]));
