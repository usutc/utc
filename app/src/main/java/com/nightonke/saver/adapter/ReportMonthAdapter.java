...
public class ReportMonthAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        name.setText(UtcCoinUtil.GetMonthShort((int) highestMonthExpense.get(position + 1)[1] + 1) + " " + year + UtcCoinUtil.getInstance().GetPurePercentString(highestMonthExpense.get(position + 1)[4] * 100));
        expense.setText(UtcCoinUtil.getInstance().GetInMoney((int) highestMonthExpense.get(position + 1)[3]));
        records.setText(UtcCoinUtil.getInstance().GetInRecords((int) highestMonthExpense.get(position + 1)[5]));
