...
public class DialogSelectListDataAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        month.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        year.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        expense.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        sum.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        ...
        if (monthNum == -1) {
            ...
            month.setText(UtcCoinUtil.getInstance().GetMonthShort((int)data.get(position)[1]));
            year.setText((int)data.get(position)[0] + "");
        }
        expense.setText(UtcCoinUtil.GetInMoney((int)data.get(position)[3]));
