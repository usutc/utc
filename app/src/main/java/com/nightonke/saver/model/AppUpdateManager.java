...
public class AppUpdateManager {
    ...
    private static final String FILE_NAME = FILE_PATH + "UtcCoin.apk";
    ...
    public void checkUpdateInfo(final Boolean showInfo) {
        ...
        query.addWhereGreaterThan("version", UtcCoinApplication.VERSION);
        query.setLimit(Integer.MAX_VALUE);
        query.findObjects(UtcCoinApplication.getAppContext(), new FindListener<APK>() {
            @Override
            public void onSuccess(final List<APK> object) {
                if (object.size() == 0 && showInfo) {
                    UtcCoinUtil.showToast(context, context.getResources().getString(R.string.is_newest_version), SuperToast.Background.BLUE);
                }
                BmobQuery<APK> tooOldQuery = new BmobQuery<>();
                tooOldQuery.addWhereEqualTo("version", UtcCoinApplication.VERSION);
                tooOldQuery.setLimit(1);
                tooOldQuery.findObjects(UtcCoinApplication.getAppContext(), new FindListener<APK>() {
                    @Override
