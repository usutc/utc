...
public class UtcCoinRecord extends BmobObject {
    ...
    public UtcCoinRecord() {
        this.id = Long.valueOf(-1);
    }

    public UtcCoinRecord(long id, float money, String currency, int tag, Calendar calendar) {
        this.id = id;
        this.money = money;
    ...
    public UtcCoinRecord(long id, float money, String currency, int tag, Calendar calendar, String remark) {
        this.id = id;
        this.money = money;
        this.currency = currency;
        this.tag = tag;
    ...
    public void set(UtcCoinRecord coCoinRecord) {
    ...
    public boolean isInMoney(double money1, double money2, String currency) {
        return UtcCoinUtil.ToDollas(money1, currency) <= UtcCoinUtil.ToDollas(this.money, this.currency)
                && UtcCoinUtil.ToDollas(money2, currency) > UtcCoinUtil.ToDollas(this.money, this.currency);
    }

    public String getCalendarString() {
        return String.format("%02d", calendar.get(Calendar.HOUR_OF_DAY)) + ":"
                + String.format("%02d", calendar.get(Calendar.MINUTE)) + " "
                + UtcCoinUtil.GetMonthShort(calendar.get(Calendar.MONTH) + 1) + " "
