...
public class TodayViewFragment extends Fragment {
    ...
    private List<UtcCoinRecord> list = new ArrayList<>();
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        switch (position) {
            case TODAY:
                leftRange = UtcCoinUtil.GetTodayLeftRange(now);
                ...
                break;
            case YESTERDAY:
                leftRange = UtcCoinUtil.GetYesterdayLeftRange(now);
                rightRange = UtcCoinUtil.GetYesterdayRightRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
                ...
                break;
            case THIS_WEEK:
                leftRange = UtcCoinUtil.GetThisWeekLeftRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
                ...
                break;
            case LAST_WEEK:
                leftRange = UtcCoinUtil.GetLastWeekLeftRange(now);
                rightRange = UtcCoinUtil.GetLastWeekRightRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
                ...
                break;
            case THIS_MONTH:
                leftRange = UtcCoinUtil.GetThisMonthLeftRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
                ...
                break;
            case LAST_MONTH:
                leftRange = UtcCoinUtil.GetLastMonthLeftRange(now);
                rightRange = UtcCoinUtil.GetLastMonthRightRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
                ...
                break;
            case THIS_YEAR:
                leftRange = UtcCoinUtil.GetThisYearLeftRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
                ...
                break;
            case LAST_YEAR:
                leftRange = UtcCoinUtil.GetLastYearLeftRange(now);
                rightRange = UtcCoinUtil.GetLastYearRightRange(now);
                for (int i = recordManager.RECORDS.size() - 1; i >= 0; i--) {
                    if (recordManager.RECORDS.get(i).getCalendar().before(leftRange)) {
                        end = i + 1;
                        break;
