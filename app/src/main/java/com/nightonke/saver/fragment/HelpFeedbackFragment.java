...
public class HelpFeedbackFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        input.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        help = (TextView)view.findViewById(R.id.helper);
        help.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        number = (TextView)view.findViewById(R.id.number);
        number.setTypeface(UtcCoinUtil.getInstance().typefaceLatoLight);
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (exceed) {
                    ...
                    UtcCoinUtil.getInstance().showToast(UtcCoinApplication.getAppContext(), UtcCoinApplication.getAppContext().getResources().getString(R.string.help_feedback_sent));
                    Feedback feedback = new Feedback();
                    feedback.setContent(input.getText().toString());
                    feedback.save(UtcCoinApplication.getAppContext(), new SaveListener() {
                        @Override
                        public void onSuccess() {
                            UtcCoinUtil.getInstance().showToast(UtcCoinApplication.getAppContext(), UtcCoinApplication.getAppContext().getResources().getString(R.string.help_feedback_sent_successfully));
                        }

                        @Override
                        public void onFailure(int code, String arg0) {
                            UtcCoinUtil.getInstance().showToast(UtcCoinApplication.getAppContext(), UtcCoinApplication.getAppContext().getResources().getString(R.string.help_feedback_sent_fail));
                        }
                    });
                }
            }
    ...
    private void setNumberText() {
        ...
        if (chineseIsDoubleCount) {
            count = UtcCoinUtil.getInstance().textCounter(input.getText().toString());
        } else {
